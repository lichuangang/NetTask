...
namespace TagBookTask.DAOS
{
    public class TagBookDao : BaseDapperDao
    {
        ...
        public static List<TagBook> GetDeletedTagBooks()
        {
            var sql = @"SELECT * from TagBooks WHERE Status=0 AND EndTime<@Now ";
            return QueryList<TagBook>(sql, new
            {
                DateTime.Now
            }).ToList();
        ...
        public static HomeBookTime GetHomeBooks(string bookId)
        {
            var sql = @"SELECT * from HomeBookTimes WHERE BookId <= @BookId ";
        ...
        public static HomeBookTime GetBookTime(string bookId)
        {
            ...
            WHERE   BookID = @BookId";
            return QueryList<HomeBookTime>(sql, new { BookId = bookId }).FirstOrDefault();
        }
